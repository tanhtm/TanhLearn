def f(bit32, key):
    ...
    # Mở rộng 32-bit input thành 48 bits và chuyển về dạng số nguyên
    bit48 = int(_expansion(bit32), 2)
    
    # XOR kết quả 48-bit với key, và chuyển lại về binary
    bit48 ^= int(key, 2)
    bit48 = bin(bit48)[2:].zfill(48)
    
    # Thực hiện thay thế với Sbox
    bit32 = _Sbox(bit48)
...
def _Sbox(bit48):
    ...
    # Tách 48-bit đầu vào thành 8 khối 6-bit
    bit6_list = []
    for i in range(0, 48, 6):
        bit6_list.append(bit48[i:i+6])

    # Tiến hành thay thế từng khối 6 bit sử dụng sbox tương ứng
    out = ""
    for i in range(8):
        chunk = bit6_list[i]
